(function(dynCore, hashNav) {
    dynCore.css('dndRef', '/hub/css/dndRef.css');
    ...
    function init(modules) {
        ...
        var nosql = modules.lib.nosql;
        var indexer = modules.lib.indexer;
        var ajaxLoader = modules.lib.ajaxLoader;
        var ajaxError = modules.lib.ajaxError;
        var delayedAction = modules.lib.delayedAction
        ...
        var dndRef = {
            ...
            mergeSearchTerms: function(data, ext) {
                for (var i = 0; i < ext.length; i++) {
                    var content = JSON.parse(ext[i].Text);
                    ...
                    for (var n = 0; n < content.length; n++) {
                        ...
                        for (var x = 0; x < data.length; x++) {
                            if (data[x].name === content[n].name ||
                                (data[x].name === 'Bestiary' &&
                                content[n].attributes.Category === 'Monsters')) {
                        ...
                        headings = [];
                        for (var y = 0; y < content[n].sections.length; y++) {
                            if (content[n].sections[y].header) {
                                headings.push(content[n].sections[y].header);
                            }
                        }

                        if (existing) {
                            if (existing.name === 'Bestiary') {
                                existing.tags.push({
                                    attributes: content[n].attributes,
                                    name: content[n].name
                                });
                            } else {
                                existing.tags = existing.tags.concat(headings);
                            }
                        } else {
                            data.push({
                                attributes: content[n].attributes,
                                name: content[n].name,
                                tags: headings,
                                url: '#dndRef-page/ref/' + content[n].name.toLocaleLowerCase()
                            });
                        }
                    }
                }
            ...
            api: {
                resources: function(metaSuffix, name) {
                    ...
                    var promise = nosql.resources(meta);
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                },

                bestiary: function() {
                    var promise = $.ajax('/hub/json/dnd/bestiary.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                ...
                spellbook: function() {
                    var promise = $.ajax('/hub/json/dnd/spells.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                ...
                elementalEvilSpells: function() {
                    var promise = $.ajax('/hub/json/dnd/ext/elemental evil spells.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                ...
                spellLevels: function() {
                    var promise = $.ajax('/hub/json/dnd/spellLevels.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                ...
                page: function(name) {
                    var promise = $.ajax('/hub/json/dnd/' + name + '.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
                ...
                directory: function() {
                    var promise = $.ajax('/hub/json/dnd/ext/directory.json');
                    pendingAjax.push(promise);
                    return promise.always(function() {
                        pendingAjax.splice(pendingAjax.indexOf(promise), 1);
                    });
            ...
            index: {
                bestiary: function() {
                    indexer.indexArray(dndRef.bestiary, dndRef.filterPaths.name);
                    indexer.indexArray(dndRef.bestiary, dndRef.filterPaths.cr);
                },

                spellbook: function() {
                    indexer.indexArray(dndRef.spellbook, dndRef.filterPaths.name);
                }
            },
            ...
            load: {
                ...
                bestiary: function($content) {
                    ...
                    dndRef.api.bestiary().done(function(data) {
                        if (centralAuth.google.info) {
                            ...
                            var fn = function() {
                                centralAuth.google.off('signIn', fn);
                            ...
                            centralAuth.google.on('signIn', fn);
                        }
                    }).fail(function() {
                        promise.reject();
                    });
                ...
                resourcePage: function($content, meta, name) {
                    ...
                    var promise = $.Deferred();
                    ajaxLoader(promise, $content).fail(function() {
            ...
            render: {
                ...
                monster: function(name) {
                    ...
                    var match = indexer.get(dndRef.bestiary, dndRef.filterPaths.name, name);
                    var filtered = indexer.filter(dndRef.bestiary, match);
                ...
                spellbook: function() {
                    ...
                    for (var i = 0; i < $spells.length; i++) {
                        var $spell = $($spells[i]);
                        var match = indexer.get(
                            dndRef.spellbook,
                            dndRef.filterPaths.name,
                            $spell.children('.name').text()
                        );
                        var filtered = indexer.filter(dndRef.spellbook, match);
            ...
            nav: {
                ...
                directory: function(search) {
                    ...
                    if (centralAuth.google.info && !search &&
                    ...
                    var template = dynCore.loadTemplate('dndRef.directory', '/hub/frag/dndRefDirectory.html');
                    var itemTemplate = dynCore.loadTemplate('dndRef.directoryItem', '/hub/frag/dndRefDirectoryItem.html');
                ...
                bestiary: function(category, minCR, maxCR, query) {
                    ...
                    var template = dynCore.loadTemplate('dndRef.bestiary', '/hub/frag/dndRefBestiary.html');
                ...
                monster: function(name) {
                    ...
                    var template = dynCore.loadTemplate('dndRef.monster', '/hub/frag/dndRefMonster.html');
                    $.when(dndRef.load.bestiary($content), template).done(function() {
                ...
                spellbook: function(className, level, spell) {
                    ...
                    var template = dynCore.loadTemplate('dndRef.spell', '/hub/frag/dndRefSpell.html');
                ...
                page: function(name, mod) {
                    ...
                    var template = dynCore.loadTemplate('dndRef.page', '/hub/frag/dndRefPage.html');
                    if (mod) {
                        if (centralAuth.google.info) {
        ...
        $('#dndRef-bestiary input').on('keyup', function() {
            delayedAction(dndRef.rehash.bestiary, 'bestiarySearch');
        });

        $('#dndRef-spellbook select').on('change', dndRef.rehash.spellbook);
        $('#dndRef-spellbook input').on('keyup', function() {
            delayedAction(dndRef.rehash.spellbook, 'spellbookSearch');
        });

        $('#app-dndRef .dndRefDirectorySearch').on('keyup', function() {
            delayedAction(dndRef.rehash.directory, 'directorySearch');
        });
        
        centralAuth.google.on('signIn', dndRef.signInOut);
        centralAuth.google.on('signOut', dndRef.signInOut);
