(function(dynCore, hashNav) {
    dynCore.css('pinboard', '/hub/css/pinboard.css');
    ...
    dynCore.when(dynCore.html('pinboard'),
        ...
        dynCore.js('vendor.Sortable'),
        dynCore.js('vendor.Clamp')
    ...
    function init(modules) {
        ...
        var pinboard = {
            ...
            userIsOwner: function() {
                try {
                    if (lib.centralAuth.google.info.id &&
                        pinboard.boards[pinboard.nav.currentView.id].UserId ===
                        lib.centralAuth.google.info.id) {
            ...
            makeDeletable: function($items, fnGetTitle, fnOnConfirm) {
                ...
                if ($self.hasClass('alert')) {
                    ...
                    $items.on('click', function(e) {
                        ...
                        var promise = lib.twoButtonDialog('Really delete ' + title + '?',
            ...
            serializeCurrent: function() {
                ...
                $('#app-pinboard .pinboardTile').each(function(i, pin) {
                    ...
                    var background = {
                        'data-image': $background.attr('data-image'),
                        'data-color': $background.attr('data-color')
                    };
            ...
            deserialize: function(json) {
                ...
                if (contents.length) {
                    return contents.map(function(pin) {
                        var background = pin.background
                        background.style = '';
                        if (background['data-image']) {
                            background.style += 'background-image: url("' + background['data-image'] + '");'
                        }
                        if (background['data-color']) {
                            background.style += 'background-color: ' + background['data-color'] + ';';
                        }
            ...
            selectPin: function(pin) {
                ...
                if (lib.isInt(pin) && pin < $pins.length) {
            ...
            api: {
                save: function(board, label) {
                    ...
                    return lib.nosql.save('pinboard', {
                        Id: board.id,
                        Text: JSON.stringify(board.pins),
                        Label: board.label,
                ...
                delete: function(id) {
                    return lib.nosql.delete('pinboard', id).done(function(id) {
                ...
                singleBoard: function(id) {
                    return lib.nosql.get('pinboard', id);
                },

                privateBoards: function() {
                    return lib.nosql.get('pinboard');
                },

                publicBoards: function(userId) {
                    return lib.nosql.get('pinboard/public', userId);
                }
            },
            ...
            refresh: {
                view: function(id, pin) {
                    ...
                    var tileTemplate = dynCore.loadTemplate('pinboard.tile', '/hub/frag/pinboardTile.html');
                    var contentTemplate = dynCore.loadTemplate('pinboard.pinContent', '/hub/frag/pinboardTileContent.html');

                    var fnRetry = function() {
                        lib.ajaxError($board, function() {
                            pinboard.refresh.view(id, pin);
                        });
                    };

                    if (!id) {
                        var newBoard = lib.ajaxLoader(pinboard.api.save({ pins: [
                            {
                                background: {
                                    'data-color': '#888;'
                                },
                        ...
                        board = lib.ajaxLoader(pinboard.api.singleBoard(id), $board);

                        var fnWaitForAuth = function() {
                            var fn = function(info) {
                                lib.centralAuth.google.off('signIn', fn);
                                pinboard.refresh.view(id, pin);
                            }
                            lib.centralAuth.google.on('signIn', fn);
                        }

                        board.fail(function() {
                            if (!lib.centralAuth.google.info) {
                                fnWaitForAuth();
                                return;
                            }
                        });
                        ...
                        $.when(board, tileTemplate, contentTemplate).done(function(resp) {
                            var board = resp[0];
                            if (!lib.centralAuth.google.info && !board.Public) {
                                fnWaitForAuth();
                                return;
                            }
                ...
                privateBoards: function() {
                    ...
                    var boards = lib.ajaxLoader(pinboard.api.privateBoards(), $privateBoards);
                    var template = dynCore.loadTemplate('pinboard.boardListItem', '/hub/frag/pinboardBoardListItem.html');
                    ...
                    $.when(boards, template).done(function(boards) {
                        ...
                        lib.ajaxError($privateBoards, function() {
                            pinboard.refresh.privateBoards();
                        });
                    });
                },
                ...
                publicBoards: function(userId) {
                    ...
                    var boards = lib.ajaxLoader(pinboard.api.publicBoards(userId), $publicBoards);
                    var template = dynCore.loadTemplate('pinboard.boardListItem', '/hub/frag/pinboardBoardListItem.html');
                    ...
                    $.when(boards, template).done(function(boards) {
                        ...
                        lib.ajaxError($publicBoards, function() {
            ...
            nav: {
                ...
                load: function(publicOrPrivate, userId) {
                    ...
                    if (lib.centralAuth.google.info) {
                        pinboard.signIn.load(lib.centralAuth.google.info);
                    } else {
                        $('#app-pinboard .top-bar-right .mainActions').hide();
                    }
            ...
            makePinContent: function(promise, $content, content) {
                ...
                var template = dynCore.loadTemplate('pinboard.pinContent', '/hub/frag/pinboardTileContent.html');
            ...
            getRoles: function() {
                var $roleList = $('#pinboard-view-options .roleList');
                var promise = lib.ajaxLoader($.Deferred(), $roleList.parent());
                $roleList.empty();

                var getRoles = lib.nosql.get('pinboard', pinboard.nav.currentView.id, 'roles');
                var template = dynCore.loadTemplate('pinboard.roleListItem', '/hub/frag/pinboardRoleListItem.html');

                $.when(getRoles, template).done(function(roleResp) {
                    var roles = roleResp[0];
                    var args = roles.sort().map(function(role) {
                        return {
                            '.label': {
                                text: role,
                            },
                    ...
                    var listItems = dynCore.makeFragment('pinboard.roleListItem', args);
                    var self = this;
                    listItems.find('.delete').on('click', function() {
                        var $self = $(this);
                        var role = $self.data('role');
                        $self.addClass('disabled');
                        lib.nosql.delete('pinboard', pinboard.nav.currentView.id, 'roles', role).done(function() {
                            pinboard.getRoles();
                        }).fail(function() {
        ...
        lib.centralAuth.google.on('signIn', function(info) {
            for (let e in pinboard.signIn) {
                pinboard.signIn[e](info);
            }
        });

        lib.centralAuth.google.on('signOut', function() {
        ...
        $('#pinboard-view-settings .pinboardDeletePin').on('click', function() {
            ...
            var promise = lib.twoButtonDialog('Really delete pin ' + title + '?',
                'This cannot be undone.', 'Delete', 'Cancel', true);
        ...
        $('#pinboard-view-options .addRole').on('click', function() {
            var newRole = $('#pinboard-view-options .addRoleName').val();
            var $self = $(this);
            if (newRole) {
                $self.addClass("disabled");
                var url = dynCore.getResource('node') + '/nosql/pinboard/' + pinboard.nav.currentView.id + '/roles/' + newRole;
                lib.cors({
                    url: url,
                    method: 'POST'
                }).done(function() {
        ...
        $('#app-pinboard .pinActions .pinOptions').on('click', function() {
            ...
            var image = $background.attr('data-image') || '';
            $settings.find('.tileImageSetting').val(image);

            var color = $background.attr('data-color') || '';
            $settings.find('.tileColorSetting').val(color);
        ...
        $('#pinboard-view-settings .saveTileSettings').on('click', function() {
            ...
            if ($background.attr('data-image') !== newImage) {
                $background.attr('data-image', newImage);
            ...
            if ($background.attr('data-color') !== newColor) {
                $background.attr('data-color', newColor);
